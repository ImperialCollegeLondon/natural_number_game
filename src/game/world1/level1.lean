...
prove mathematical theorems using the Lean theorem prover.
...
Let's see it in action! In the proof of the lemma lower down in this box,
click on the word `sorry` and then delete it.
When the system finishes being busy, in the box on the top right
...
level of the tutorial! Click on "next level" in the top right to go onto the second level of
