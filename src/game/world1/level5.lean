...
more preisely, by *recursion* on `b`. He first explained how to add 0 to a number:
...
Peano tells you how to add the number after `d` to `a`.
It looks like this:
...
and then move your cursor
around the various lines of the proof using the arrow keys, and you can see what
