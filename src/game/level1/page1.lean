...
You're going to prove these theorems using *tactics*. This introductory level will take you
through some of these tactics. During your proofs, your "goal" (i.e. what you're supposed
to be proving) will be displayed with  a `⊢` symbol in front of it -- look for this symbol
to find out what you're supposed to be proving. 

## Tutorial level, page 1: the `refl` tactic.
...
on the word `sorry`. When the system finishes being busy, in the box on the right you can see your
goal -- the objective of this level. The goal in this case is `x = x`.
That's a pretty easy goal to prove -- you can just prove it with the `refl` tactic.
Delete `sorry` and change it to `refl,` (**don't forget the comma**). Then hit enter
to go onto the next line.
-/
